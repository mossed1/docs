...
3. Install required dependencies and setup I2S amplifier:
   ```bash
   source TARS-AI/src/.venv/bin/activate (if not already activate)
...
5. After rebooting, activate the environment and re-run the setup script:
   ```bash
   source TARS-AI/src/.venv/bin/activate (if not already activate)
   sudo -E env PATH=$PATH python3 i2samp.py
   ```
