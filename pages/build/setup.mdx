...
## Fix VNC for SPI Displays
Since your LCD is SPI-based, you can create a fake virtual screen for VNC, so that when you VNC in, you get a higher resolution.

### 1. Install `xserver-xorg-video-dummy`
This package allows the system to create a virtual display without requiring an HDMI screen.

```bash
sudo apt install xserver-xorg-video-dummy -y
```

### 2. Create a New Xorg Configuration File
Open the config file for the virtual screen:

```bash
sudo nano /etc/X11/xorg.conf.d/99-fake-screen.conf
```
...
Section "Monitor"
    ...
    Modeline "1024x600" 50.00 1024 1056 1120 1200 600 603 613 624 -hsync +vsync
EndSection
...
This creates a virtual display at **1024x600**.
If you want a different resolution, change **"1024x600"** in the `Modeline` and `Modes` line.

### 3. Restart Xorg and VNC
After adding this, restart the system to apply the new settings:

```bash
sudo reboot
```

Now, when you VNC in, it should show **1024x600** (or whatever resolution you set).
